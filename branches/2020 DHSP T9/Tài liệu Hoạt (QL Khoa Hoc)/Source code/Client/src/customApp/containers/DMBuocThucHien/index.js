...
import actions from "../../redux/DMBuocThucHien/actions";
import {changeUrlFilter, getFilterData, getDefaultPageSize} from "../../../helpers/utility";
import queryString from 'query-string';
import api from "./config";
import {ModalAddEdit} from "./modalAddEditBuocThucHien";

const {Search} = Input;

class DMBuocThucHien extends Component {
  constructor(props) {
    document.title = 'Danh mục bước thực hiện';
    super(props);
  ...
  deleteBuocThucHien = () => {
    ...
    Modal.confirm({
      title: 'Thông báo',
      content: 'Bạn chắc chắn muốn xóa bước thực hiện này không ?',
      okText: 'Có',
      cancelText: 'Không',
      onOk: () => {
        api.XoaBuocThucHien({ListID: selectedRowKeys})
          .then(response => {
  ...
  editBuocThucHien = () => {
    ...
    modalKey++;
    const BuocThucHienID = selectedRowKeys[0];
    this.setState({loading: true});
    api.ChiTietBuocThucHien({BuocThucHienID: BuocThucHienID})
      .then(response => {
  ...
  submitModalAddEdit = (data) => {
    ...
    this.setState({loading: true}, () => {
      if (actions === 'add') {
        delete data.BuocThucHienID;
        api.ThemBuocThucHien(data)
          .then(response => {
            if (response.data.Status === 1) {
              message.destroy();
  ...
  render() {
    ...
    const {DanhSachBuocThucHien, TotalRow, TableLoading, role} = this.props;
    ...
    return (
      <LayoutWrapper>
        <PageHeader>Danh mục bước thực hiện</PageHeader>
        <PageAction>
          {role.delete ? <Button type={'primary'} style={{marginRight: 5}} onClick={this.deleteBuocThucHien}
                                 disabled={selectedRowKeys.length < 1}>Xóa</Button> : ""}
          {role.edit ? <Button type={'primary'} style={{marginRight: 5}} onClick={this.editBuocThucHien}
                               disabled={selectedRowKeys.length !== 1}>Sửa</Button> : ""}
          {role.add ? <Button type={'primary'} onClick={this.addBuocThucHien}>Thêm mới</Button> : ""}
        </PageAction>
        <Box>
          <BoxFilter>
            <Search placeholder={'Nhập tên bước thực hiện cần tìm kiếm'} style={{width: 300, marginLeft: 10}}
                    allowClear
          ...
          <BoxTable
            ...
            columns={column}
            rowKey={'BuocThucHienID'}
            dataSource={DanhSachBuocThucHien}
            pagination={{
