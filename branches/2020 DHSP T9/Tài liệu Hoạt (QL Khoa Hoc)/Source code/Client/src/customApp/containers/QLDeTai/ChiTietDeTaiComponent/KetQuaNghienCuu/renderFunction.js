...
export function renderQuaTrinhDaoTao(QuaTrinhDaoTao, editFunction, deleteFunction, disableEdit = true, disableDelete = true) {
  return (
      ...
      {QuaTrinhDaoTao.map((item, index) => (
        <tr>
          ...
          <td style={{ textAlign: "center" }}>
            <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 1)} />
            <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 1)} />
          </td>
        </tr>
      ))}
    </table>
      ...
      {QuaTrinhCongTac.map((item, index) => {
        return (
          <tr>
            ...
            <td>{item.ChucVu}</td>
            <td style={{ textAlign: "center" }}>
              <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 2)} />
              <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 2)} />
            </td>
          </tr>
        );
      })}
      ...
      {NgoaiNgu.map((item, index) => {
        return (
          <tr>
            ...
            <td style={{ textAlign: "center" }}>
              <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 3)} />
              <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 3)} />
            </td>
          </tr>
        );
      })}
        ...
        return (
          <tr style={{ height: 35 }}>
            <td style={{ textAlign: "center", width: "10%" }}>
              <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 4)} />
              <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 4)} />
            </td>
            <td style={{ textAlign: "center", width: "20%" }}>{item.NgayCap ? moment(item.NgayCap).format("DD/MM/YYYY") : ""}</td>
            <td>
              {item.TieuDe}
              {item.TrinhDo !== "" ? ` (${item.TrinhDo})` : ""}
              {item.NoiCap !== "" ? ", " : ""}
              {item.NoiCap}
              {item.SoHieu !== "" ? ", " : ""}
              {item.SoHieu}
    ...
    <table style={{ width: "100%" }}>
      {GiaiThuongKhoaHoc.map((item, index) => {
        return (
          <tr style={{ height: 35 }}>
            <td style={{ textAlign: "center", width: "10%" }}>
              <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 5)} />
              <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 5)} />
            </td>
            <td style={{ textAlign: "center", width: "20%" }}>{item.KhoangThoiGian}</td>
            <td>{item.TieuDe}</td>
          </tr>
        );
      })}
      ...
      {DuAnDeTai.map((item, index) => {
        return (
          <tr>
            ...
            <td style={{ textAlign: "center" }}>
              {!item.Disable ? <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 6)} /> : ""}
              {!item.Disable ? <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 6)} /> : ""}
            </td>
          </tr>
        );
      })}
...
export function renderBaiBaoTapChi(BaiBaoTapChi, editFunction, deleteFunction, disableEdit = false, disableDelete = false, DanhSachCanBo = []) {
  return (
    <table style={{ width: "100%" }}>
      {BaiBaoTapChi.map((item, index) => {
        const isAuthor = JSON.parse(localStorage.getItem("user")).CanBoID === item.NguoiTaoID;
        return (
          <tr style={{ height: 35 }}>
            <td style={{ textAlign: "center", width: "10%" }}>
              <Icon
                ...
                className={!isAuthor ? "not-allow" : "pointer"}
                onClick={() => {
                  if (!isAuthor) {
                    return;
                  } else {
                    editFunction(index, 7);
                  }
                }}
              />
              <Icon
                ...
                onClick={() => {
                  if (!isAuthor) {
                    ...
                    deleteFunction(item.ChiTietDeTaiID, 7);
                  }
                }}
              />
            </td>
            <td style={{ textAlign: "center", width: "20%" }}>{item.NamDangTai}</td>
            <td>
              {item.LinkBaiBao !== "" ? (
                <a href={item.LinkBaiBao} target={"_blank"}>
                  {renderContentBaiBao(item, DanhSachCanBo)}
                </a>
              ) : (
                renderContentBaiBao(item, DanhSachCanBo)
              )}
              ...
              <Icon
                ...
                onClick={() => {
                  if (!isAuthor) {
                    ...
                    deleteFunction(item.ChiTietDeTaiID, 8);
                  }
                }}
              />
            </td>
            <td style={{ textAlign: "center", width: "20%" }}>{item.NamXuatBan}</td>
            <td>{renderContentKetQuaNghienCuu(item, DanhSachCanBo)}</td>
          </tr>
        );
      })}
    </table>
              ...
              <Icon
                ...
                onClick={() => {
                  if (!isAuthor) {
                    ...
                    deleteFunction(item.ChiTietDeTaiID, 9);
                  }
                }}
              />
            </td>
            <td style={{ textAlign: "center", width: "20%" }}>{item.NamXuatBan}</td>
            <td>{renderContentSachChuyenKhao(item, DanhSachCanBo)}</td>
          </tr>
        );
      })}
    </table>
...
export function renderSanPhamDaoTao(SanPhamDaoTao, editFunction, deleteFunction, disableEdit, disableDelete) {
  ...
  //                   return;
  //                 } else {
  //                   deleteFunction(item.ChiTietDeTaiID, 10);
  //                 }
  //               }}
  //             />
  //           </td>
  //           <td style={{ textAlign: "center", width: "20%" }}>{item.NamBaoVe}</td>
  //           <td>{renderContentSanPhamDaoTao(item)}</td>
  //         </tr>
  //       );
  //     })}
  //   </table>
  ...
  return (
    <table className={"table-modal"}>
      <tr>
        ...
        <th rowSpan={2} style={{ width: "15%" }}>
          Tên khóa luận/ luận văn/ luận án
        </th>
      ...
      {SanPhamDaoTao.map((item, index) => (
        <tr>
          ...
          <td style={{ textAlign: "center" }}>
            {!item.Disable
              ? [
                  <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 10)} />,
                  <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(item.ChiTietDeTaiID, 10)} />,
                ]
              : ""}
          </td>
        </tr>
      ...
      {CacMonGiangDay.map((item, index) => {
        return (
          <tr style={{ height: 35 }}>
            <td style={{ textAlign: "center", width: "10%" }}>
              <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 10)} />
              <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 10)} />
            </td>
            <td style={{ cursor: item.FileDinhKem.length > 0 ? "pointer" : "unset" }}>
              {item.FileDinhKem.length > 0 ? (
                <Popover content={renderContentDownload(item.FileDinhKem)}>
                  {item.TieuDe}
                  {item.DeCuong !== "" ? ` (${item.DeCuong}) ` : ""}
                </Popover>
              ) : (
                <span>
                  {item.TieuDe}
      ...
      {HoatDongKhoaHoc.map((item, index) => {
        return (
          <tr>
            ...
            <td style={{ textAlign: "center" }}>{item.PublicCV ? <Icon type={"check"} /> : ""}</td>
            <td style={{ textAlign: "center" }}>
              <Icon type={"edit"} style={{ marginRight: 10 }} className={disableEdit ? "not-allow" : "pointer"} onClick={() => editFunction(index, 11)} />
              <Icon type={"delete"} className={disableDelete ? "not-allow" : "pointer"} onClick={() => deleteFunction(index, 11)} />
            </td>
          </tr>
        );
      })}
...
export function logoHNUEBase64() {
  ...
  if (hostPath.includes("local")) {
    hostPath = "http://192.168.100.46:7001";
  } else {
...
export function renderContentBaiBao(item, DanhSachCanBo) {
  ...
  if (item.LoaiBaiBao === 1) {
    ...
    return [ListTenTacGiaStr, item.TieuDe, [item.TenTapChiSachHoiThao, subContent].filter((item) => item !== "").join(" "), soISSN, item.Tap, item.So, item.Trang, item.NhaXuatBan]
      .filter((item) => item !== "")
      .join(", ");
    ...
    return [
      ListTenTacGiaStr,
      item.TieuDe,
      [item.TenTapChiSachHoiThao, subContent].filter((item) => item !== "").join(" "),
      soISSN,
      item.Tap,
      item.So,
      item.Trang,
