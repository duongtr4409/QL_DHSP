...
export default class TableData extends React.Component {
  ...
  openModal = (data = null) => {
    this.modal = Modal.confirm({
      icon: <i />,
      content: <ModalComponent refresh={this.props.refresh} DeTaiID={this.props.DeTaiID} data={data} onClose={this.handleCloseModal}></ModalComponent>,
    });
  };
  handleCloseModal = () => {
    this.modal.destroy();
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: "Nội dung đã làm",
        dataIndex: "NoiDungDaLam",
        key: "NoiDungDaLam",
      },
      {
        title: "Sản phẩm",
...

      {
        ...
        width: "150px",
        render: (text, record, index) => {
          if (record.NguoiTaoID !== JSON.parse(localStorage.getItem("user")).CanBoID) {
            return null;
          }
          return (
