...
import api from "../../containers/DMLinhVuc/config";
...
function* getInitData({ payload }) {
  try {
    ...
    yield put({
      ...
      payload: {
        DanhSachLinhVuc: resultData.DanhSachLinhVuc,
...
function formatData(DanhSachLinhVuc) {
  let expandedKeys = [];
  const DanhSach = DanhSachLinhVuc.map((value1, index1) => {
    //-------1
  ...
  return {
    DanhSachLinhVuc: DanhSach,
    expandedKeys,
  };
}

export default function* rootSaga() {
  yield all([yield takeEvery(actions.LINHVUC_GET_INIT_DATA_REQUEST, getInitData)]);
