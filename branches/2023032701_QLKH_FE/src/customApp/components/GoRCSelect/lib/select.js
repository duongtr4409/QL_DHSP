...
class SuperSelect extends PureComponent {
  constructor(props) {
    ...
    this.id = this.props.id || `sid${+new Date()}`;
  }
  ...
  onScroll = () => {
    return this.throttleByHeight(this.onScrollReal);
  };
  ...
  render() {
    let { dropdownStyle, optionLabelProp, notFoundContent, dropdownClassName, ...props } = this.props;

    this.allList = this.getUseChildrenList();

    this.allHeight = this.allList.length * this.ITEM_HEIGHT - 60 || 100;
    const { startIndex, endIndex } = this.getStartAndEndIndex();
      ...
      <Select
        ...
        dropdownRender={(menu) => {
          ...
          if (this.allList.length === 0) {
            tempMenu = <div style={{ padding: "5px 12px" }}>Không có dữ liệu</div>;
          } else {
            tempMenu = (
              <DropDownWrap
                {...{
          ...
          return (
            <div className="position-relative">
              ...
              <div style={{ position: "relative" }} className="text-primary">
                <Divider style={{ margin: "4px 0" }} />
                <div style={{ padding: "4px 8px", cursor: "pointer" }} onMouseDown={(e) => e.preventDefault()} onClick={this.props.onClickCreateBtn}>
